...
public class Car : MonoBehaviour
{
	...
	private void Audio()
	{
		...
		if (!(Mathf.Lerp(this.accelerate.pitch, 0.65f + Mathf.Clamp(Mathf.Abs(this.speed / 160f), 0f, 1f), Time.deltaTime * 5f) < 0))
		{
		...
		if (!this.grounded)
		{
			this.accelerate.pitch = Math.Clamp(Mathf.Lerp(this.accelerate.pitch, 1.5f, Time.deltaTime * 8f), 0, 1000000);
		}
		this.deaccelerate.pitch = Math.Clamp(Mathf.Lerp(this.deaccelerate.pitch, 0.5f + this.speed / 40f, Time.deltaTime * 2f), 0, 10000000);
	}
